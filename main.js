...
// https://raw.githubusercontent.com/AvidGameFan/ed-plugins/master/OutpaintIt.plugin.js
...
//     const outpaintPercentToKeep = 0.3; //Amount of random pixels to retain, to bias effect
//     const maskFade = 0.07;
...
//             //approximation of Gaussian, from Stackoverflow
//             var rand = 0;
//             for (var i = 0; i < 6; i += 1) {
//                 rand += Math.random();
//             }
...
//             64 *
//                 Math.floor((image.naturalWidth * image.naturalHeight) / 2000000)
//         );
...
//         let initialPromptStrength = OutpaintItSettings.useExtendImage
//             ? 0.89
//             : 0.95;
...
//             prompt_strength:
//                 initialPromptStrength +
//                 Math.random() * (1 - initialPromptStrength), //be sure the values add up to 1 or less
//             width:
//                 image.naturalWidth +
//                 (widen || all ? calcOutpaintSizeIncrease(image) : 0),
//             height:
//                 image.naturalHeight +
//                 (!widen || all ? calcOutpaintSizeIncrease(image) : 0),
...
//         if (
//             newTaskRequest.reqBody.width * newTaskRequest.reqBody.height >
//             outpaintMaxTurboResolution
//         ) {
//             //put max normal resolution here
...
//         if (
//             newTaskRequest.reqBody.prompt.substr(
//                 0,
//                 $('textarea#prompt').val().length
//             ) != $('textarea#prompt').val()
//         ) {
...
//             newTaskRequest.reqBody.use_stable_diffusion_model = $(
//                 '#editor-settings #stable_diffusion_model'
//             )[0].dataset.path;
//             //newTaskRequest.reqBody.use_stable_diffusion_model =$("#editor-settings #stable_diffusion_model")[0].dataset.path;
//             newTaskRequest.use_vae_model = $(
//                 '#editor-settings #vae_model'
//             )[0].dataset.path;
//         }
...
//             html: '<i class="fa-solid fa-thumbs-up"></i> Like',
//             type: 'label',
//             on_click: onOutpaintLabelClick,
//             filter: onOutpaintLabelFilter,
//         },
//         {
//             html: '<i class="fa-solid fa-arrow-up"></i>',
//             on_click: onOutpaintUpClick,
//             filter: onOutpaintUpFilter,
//         },
//         {
//             html: '<i class="fa-solid fa-arrow-down"></i>',
//             on_click: onOutpaintDownClick,
//             filter: onOutpaintDownFilter,
//         },
//         {
//             html: '<i class="fa-solid fa-arrow-left"></i>',
//             on_click: onOutpaintLeftClick,
//             filter: onOutpaintLeftFilter,
//         },
//         {
//             html: '<i class="fa-solid fa-arrow-right"></i>',
//             on_click: onOutpaintRightClick,
//             filter: onOutpaintRightFilter,
//         },
//         {
//             html: '<i class="fa-solid fa-arrows"></i>',
//             on_click: onOutpaintAllClick,
//             filter: onOutpaintAllFilter,
//         },
//     ]);
...
//                 0,
//                 image.naturalWidth,
//                 calcOutpaintSizeIncrease(image), //source
//                 0,
//                 image.naturalHeight,
//                 image.naturalWidth,
//                 calcOutpaintSizeIncrease(image) //destination -- inverted
//             );
//             ctx.restore();
...
//             0,
//             canvas.width,
//             calcOutpaintSizeIncrease(image)
//         );
...
//             calcOutpaintSizeIncrease(image),
//             image.naturalWidth,
//             image.naturalHeight //destination
//         );
...
//             image.naturalWidth,
//             calcOutpaintSizeIncrease(image) +
//                 maskExtraOverlap /*outpaintMaskOverlap*/
//         ); //Need some overlap on the mask (minimum of 8px)
...
//         if (
//             (origRequest.height +
//                 calcOutpaintSizeIncrease({
//                     naturalWidth: origRequest.width,
//                     naturalHeight: origRequest.height,
//                 })) *
//                 origRequest.width <=
//             outpaintMaxTotalResolution
//         ) {
//             result = true;
//         }
//         return result;
...
//                 image,
//                 0,
//                 image.naturalHeight - calcOutpaintSizeIncrease(image),
//                 image.naturalWidth,
//                 calcOutpaintSizeIncrease(image), //source
//                 0,
//                 0,
...
//         var imageData = ctx.getImageData(
//             0,
//             canvas.height - calcOutpaintSizeIncrease(image),
//             canvas.width,
//             calcOutpaintSizeIncrease(image)
//         );
...
//         maskctx.fillRect(
//             0,
//             image.naturalHeight - maskExtraOverlap,
//             image.naturalWidth,
//             calcOutpaintSizeIncrease(image) + maskExtraOverlap
//         ); //Need some overlap on the mask (minimum of 8px)
...
//                 0,
//                 calcOutpaintSizeIncrease(image),
//                 image.naturalHeight //destination -- inverted
//             );
//             ctx.restore();
...
//                 maskExtraOffset,
//             0,
//             image.naturalWidth - maskExtraOverlap - maskExtraOffset,
//             image.naturalHeight
//         );
...
//             ctx.drawImage(
//                 image,
//                 image.naturalWidth - calcOutpaintSizeIncrease(image),
//                 0,
//                 calcOutpaintSizeIncrease(image),
//                 image.naturalHeight, //source
//                 0,
...
//         // get the image data of the canvas  -- we only need the part we're going to outpaint
//         var imageData = ctx.getImageData(
//             canvas.width - calcOutpaintSizeIncrease(image),
//             0,
//             calcOutpaintSizeIncrease(image),
//             canvas.height
//         );
...
//         maskctx.fillStyle = 'white';
//         maskctx.fillRect(
//             image.naturalWidth - maskExtraOverlap,
//             0,
//             calcOutpaintSizeIncrease(image) + maskExtraOverlap,
//             image.naturalHeight
//         ); //Need some overlap on the mask (minimum of 8px)
...
//             image.naturalWidth -
//                 maskExtraOverlap -
//                 maskExtraOffset /*maskcanvas.width-calcOutpaintSizeIncrease(image)-8*/,
//             0,
//             0,
//             0
//         ); //horizontal line
...
//         let newTaskRequest = outpaintGetTaskRequest(
//             origRequest,
//             image,
//             true,
//             true
...
//             image.naturalWidth,
//             image.naturalHeight, //source
//             calcOutpaintSizeIncrease(image) / 2,
//             calcOutpaintSizeIncrease(image) / 2,
//             image.naturalWidth,
//             image.naturalHeight //destination
//         );
...
//         maskctx.rect(
//             maskcanvas.width -
//                 (calcOutpaintSizeIncrease(image) / 2 + maskExtraOverlap / 2),
//             calcOutpaintSizeIncrease(image) / 2 + maskExtraOverlap / 2,
//             -(image.naturalWidth - maskExtraOverlap),
//             image.naturalHeight - maskExtraOverlap,
//             true
...
//             calcOutpaintSizeIncrease(image) / 2 + maskExtraOverlap / 2,
//             image.naturalWidth - maskExtraOverlap,
//             image.naturalHeight - maskExtraOverlap - maskExtraOffset
//         );
//         //bottom
...
//         let result =
//             onOutpaintRightFilter(origRequest, image) ||
//             onOutpaintUpFilter(origRequest, image);
...
//           <input id="outpaintit_change_model" name="outpaintit_change_model" type="checkbox" value="` +
//             OutpaintItSettings.useChangedModel +
//             `"  onchange="setOutpaintItSettings()"> <label for="outpaintit_change_model"></label>
//           </div>
//           <label for="outpaintit_change_model">Use model selected above <small>(not the original model)</small></label>
//           </li>
//           <li class="pl-5"><div class="input-toggle">
//           <input id="outpaintit_change_prompt" name="outpaintit_change_prompt" type="checkbox" value="` +
//             OutpaintItSettings.useChangedPrompt +
//             `"  onchange="setOutpaintItSettings()"> <label for="outpaintit_change_prompt"></label>
//           </div>
//           <label for="outpaintit_change_prompt">Use new prompt, above <small>(not the original prompt)</small></label>
//           </li>
//           <li class="pl-5"><div class="input-toggle">
//           <input id="outpaintit_extend_image" name="outpaintit_extend_image" type="checkbox" value="` +
//             OutpaintItSettings.useExtendImage +
//             `"  onchange="setOutpaintItSettings()"> <label for="outpaintit_extend_image"></label>
//           </div>
...
//         editorSettings.parentNode.insertBefore(
//             outpaintSettings,
//             editorSettings.nextSibling
//         );
