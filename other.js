...
const taskConfigSetup = {
    taskConfig: {
        ...
        dimensions: {
            value: ({reqBody}) => `${reqBody?.width}x${reqBody?.height}`,
            label: 'Dimensions',
        },
        ...
        tiling: {
            ...
            visible: ({reqBody}) =>
                reqBody?.tiling != 'none' &&
                reqBody?.tiling !== null &&
                reqBody?.tiling !== undefined,
            value: ({reqBody}) => reqBody?.tiling,
        },
        use_vae_model: {
            label: 'VAE',
            visible: ({reqBody}) =>
                reqBody?.use_vae_model !== undefined &&
                reqBody?.use_vae_model.trim() !== '',
        },
        negative_prompt: {
            label: 'Negative Prompt',
            visible: ({reqBody}) =>
                reqBody?.negative_prompt !== undefined &&
                reqBody?.negative_prompt.trim() !== '',
        },
        prompt_strength: 'Prompt Strength',
        use_face_correction: 'Fix Faces',
        upscale: {
            value: ({reqBody}) =>
                `${reqBody?.use_upscale} (${reqBody?.upscale_amount || 4}x)`,
            label: 'Upscale',
        ...
        use_lora_model: {
            label: 'Lora Model',
            visible: ({reqBody}) => !!reqBody?.use_lora_model,
        },
        lora_alpha: {
            label: 'Lora Strength',
            visible: ({reqBody}) => !!reqBody?.use_lora_model,
...
let recentResolutionsButton = document.querySelector(
    '#recent-resolutions-button'
);
let recentResolutionsPopup = document.querySelector(
    '#recent-resolutions-popup'
);
...
let applyColorCorrectionField = document.querySelector(
    '#apply_color_correction'
);
let strictMaskBorderField = document.querySelector('#strict_mask_border');
let colorCorrectionSetting = document.querySelector(
    '#apply_color_correction_setting'
);
let strictMaskBorderSetting = document.querySelector(
...
let gfpganModelField = new ModelDropdown(
    document.querySelector('#gfpgan_model'),
    ['gfpgan', 'codeformer'],
    '',
    false
);
...
let latentUpscalerSettings = document.querySelector(
    '#latent_upscaler_settings'
);
let latentUpscalerStepsSlider = document.querySelector(
    '#latent_upscaler_steps_slider'
);
let latentUpscalerStepsField = document.querySelector('#latent_upscaler_steps');
let codeformerFidelitySlider = document.querySelector(
    '#codeformer_fidelity_slider'
);
...
let controlnetModelField = new ModelDropdown(
    document.querySelector('#controlnet_model'),
    'controlnet',
    'None',
    false
);
let vaeModelField = new ModelDropdown(
    document.querySelector('#vae_model'),
    'vae',
    'None'
);
let loraModelField = new MultiModelSelector(
    document.querySelector('#lora_model'),
    'lora',
    'LoRA',
    0.5,
    0.02
);
let hypernetworkModelField = new ModelDropdown(
    document.querySelector('#hypernetwork_model'),
    'hypernetwork',
    'None'
);
let hypernetworkStrengthSlider = document.querySelector(
    '#hypernetwork_strength_slider'
);
let hypernetworkStrengthField = document.querySelector(
    '#hypernetwork_strength'
);
let outputFormatField = document.querySelector('#output_format');
let outputLosslessField = document.querySelector('#output_lossless');
let outputLosslessContainer = document.querySelector(
    '#output_lossless_container'
);
...
let negativeEmbeddingsButton = document.querySelector(
    '#negative-embeddings-button'
);
let embeddingsDialog = document.querySelector('#embeddings-dialog');
let embeddingsDialogCloseBtn = embeddingsDialog.querySelector(
...
let embeddingsCardSizeSelector = document.querySelector(
    '#embedding-card-size-selector'
);
let addEmbeddingsThumb = document.querySelector('#add-embeddings-thumb');
let addEmbeddingsThumbInput = document.querySelector(
    '#add-embeddings-thumb-input'
);
...
let embeddingsCollapsiblesBtn = document.querySelector(
    '#embeddings-action-collapsibles-btn'
);
...
let saveAllImagesCloseBtn = document.querySelector(
    '#download-images-close-button'
);
...
let useAsThumbDialogCloseBtn = document.querySelector(
    '#use-as-thumb-dialog-close-button'
);
let useAsThumbImageContainer = document.querySelector(
    '#use-as-thumb-img-container'
);
...
imagePreview.addEventListener('drop', function (ev) {
    ...
    while (
        moveTarget &&
        typeof moveTarget === 'object' &&
        moveTarget.parentNode !== imagePreviewContent
    ) {
    ...
    if (moveTarget) {
        ...
        if (
            ...
            childs.indexOf(movedTask) < childs.indexOf(moveTarget)
        ) {
            // Move after the target if lower than current position.
            moveTarget = moveTarget.nextSibling;
        }
    }
    const newNode = imagePreviewContent.insertBefore(
        movedTask,
        moveTarget || previewTools.nextSibling
    );
...
function setServerStatus(event) {
    ...
    if (SD.serverState.devices) {
        document.dispatchEvent(
            new CustomEvent('system_info_update', {
                detail: SD.serverState.devices,
            })
        );
    }
}
...
function showImages(reqBody, res, outputContainer, livePreview) {
    ...
    res.output.forEach((result, index) => {
        ...
        let imageItemElem =
            index < imageItemElements.length ? imageItemElements[index] : null;
        if (!imageItemElem) {
            ...
            const imageRemoveBtn = imageItemElem.querySelector(
                '.imgPreviewItemClearBtn'
            );
            let parentTaskContainer = imageRemoveBtn.closest(
                '.imageTaskContainer'
            );
            imageRemoveBtn.addEventListener('click', (e) => {
                ...
                if (allHidden === true) {
                    ...
                    if (
                        !req.isProcessing ||
                        req.batchesDone == req.batchCount
                    ) {
        ...
        imageElem.addEventListener('load', function () {
            imageItemElem.querySelector(
                '.img_bottom_label'
            ).innerText = `${this.naturalWidth} x ${this.naturalHeight}`;
        });
        ...
        if ('seed' in result && !imageElem.hasAttribute('data-seed')) {
            ...
            imageExpandBtn.addEventListener('click', function () {
                function previousImage(img) {
                    const allImages = Array.from(
                        outputContainer.parentNode.querySelectorAll(
                            '.imgItem img'
                        )
                    );
                ...
                function imageModalParameter(img) {
                    ...
                    return {
                        ...
                        previous: previousImg
                            ? () => imageModalParameter(previousImg)
                            : undefined,
                        next: nextImg
                            ? () => imageModalParameter(nextImg)
                            : undefined,
                    };
                }
            ...
            let buttons = [
                ...
                {
                    text: 'Draw another 25 steps',
                    on_click: onContinueDrawingClick,
                },
                [
                    {
                        html: '<i class="fa-solid fa-undo"></i> Undo',
                        on_click: onUndoFilter,
                    },
                    {
                        html: '<i class="fa-solid fa-redo"></i> Redo',
                        on_click: onRedoFilter,
                    },
                ...
                ],
                {
                    ...
                    filter: (req, img) =>
                        'use_embeddings_model' in req ||
                        'use_lora_model' in req,
                },
            ];
            ...
            const createButton = function (btnInfo) {
                ...
                if (btnInfo.on_click || !isLabel) {
                    newButton.addEventListener('click', function (event) {
                        btnInfo.on_click.bind(newButton)(
                            req,
                            img,
                            event,
                            tools
            ...
            buttons.forEach((btn) => {
                if (Array.isArray(btn)) {
                    btn = btn.filter(
                        (btnInfo) =>
                            !btnInfo.filter || btnInfo.filter(req, img) === true
                    );
                    if (btn.length === 0) {
                        return;
                    }
                ...
                try {
                    ...
                    console.error(
                        'Error creating image info button from plugin: ',
                        btn,
                        err
                    );
                }
...
function onUseAsThumbnailClick(req, img) {
    ...
    onUseAsThumbnailClick.croppr.options.minSize = {
        width: (384 * scale) >>> 0,
        height: (384 * scale) >>> 0,
    };
    onUseAsThumbnailClick.croppr.options.startSize = {
        width: (512 * scale) >>> 0,
        height: (512 * scale) >>> 0,
    };
    ...
    if ('use_embeddings_model' in req) {
        let embeddings = req.use_embeddings_model.map((e) =>
            e.split('/').pop()
        );
...
const Bucket = {
    ...
    getList(path) {
        return fetch(`bucket/${path}`).then((response) =>
    ...
    retrieve(path) {
        return fetch(`bucket/${path}.json`).then((response) =>
    ...
    cropImageDataUrl(
        ...
        .then(async function (blob) {
            ...
            for (let embedding of options) {
                promises.push(
                    Bucket.upload(
                        `${profileName}/${embedding.dataset['type']}/${embedding.value}.png`,
                        blob
                    )
                );
            }
        ...
        .then(() => {
            useAsThumbDialog.close();
            document.dispatchEvent(
                new CustomEvent('saveThumb', {
                    detail: useAsThumbSelect.selectedOptions,
                })
            );
        })
        .catch((error) => {
...
function moveImageBetweenBuffers(
    img,
    fromBuffer,
    toBuffer,
    fromButton,
...
function onUpscaleClick(req, img, e, tools) {
    ...
    let filterName = path.toLowerCase().includes('realesrgan')
        ? 'realesrgan'
        : 'latent_upscaler';
...
function onFixFacesClick(req, img, e, tools) {
    ...
    let filterName = path.toLowerCase().includes('gfpgan')
        ? 'gfpgan'
        : 'codeformer';
...
function makeImage() {
    ...
    if (
        controlnetModelField.value === '' &&
        IMAGE_REGEX.test(controlImagePreview.src)
    ) {
        alert('Please choose a ControlNet model, to use the ControlNet image.');
        document
            .getElementById('controlnet_model')
    ...
    if (
        numOutputsParallelField.value == '' ||
        numOutputsParallelField.value == 0
    ) {
    ...
    if (hypernetworkStrengthField.value == '') {
        hypernetworkStrengthField.value =
            hypernetworkStrengthSlider.value / 100;
    }
...
function createInitImageHover(taskEntry, task) {
    taskEntry.querySelectorAll('.task-initimg').forEach((thumb) => {
        ...
        div.querySelector('.useForControlnetBtn').addEventListener(
            'click',
            (e) => {
                e.preventDefault();
                controlImagePreview.src = img.src;
        ...
        div.querySelector('.downloadPreviewImg').addEventListener(
            'click',
            (e) => {
                e.preventDefault();
    ...
    $tooltip.append(img);
    $tooltip.append(
        `<div class="top-right"><button>Use as Input</button></div>`
    );
...
function onTaskEntryDragOver(event) {
    ...
    if (event.target.closest('.imageTaskContainer')) {
        if (startX && startY) {
            if (
                event.target.closest('.imageTaskContainer').offsetTop > startY
            ) {
                event.target
                    .closest('.imageTaskContainer')
                    .classList.add('dropTargetAfter');
            } else if (
                event.target.closest('.imageTaskContainer').offsetTop < startY
            ) {
                event.target
                    .closest('.imageTaskContainer')
                    .classList.add('dropTargetBefore');
            } else if (
                event.target.closest('.imageTaskContainer').offsetLeft > startX
            ) {
                event.target
                    .closest('.imageTaskContainer')
                    .classList.add('dropTargetAfter');
...
function getVisibleConfig(config, task) {
    ...
    return Object.keys(mergedTaskConfig)
        .map((key) => {
            const value =
                mergedTaskConfig?.[key]?.value?.(task) ?? task.reqBody[key];
            const visible =
                mergedTaskConfig?.[key]?.visible?.(task) ??
                value !== undefined ??
                true;
            const label =
                mergedTaskConfig?.[key]?.label ?? mergedTaskConfig?.[key];
...
function createTask(task) {
    ...
    taskConfig += `<div class="taskConfigData">${createTaskConfig(
        task
    )}</span></div></div>`;
    ...
    if (
        task.reqBody.init_image !== undefined ||
        task.reqBody.control_image !== undefined
    ) {
    ...
    if (
        task.reqBody.control_image !== undefined &&
        task.reqBody.control_filter_to_apply !== undefined
    ) {
        let req = {
        ...
        req['model_paths'][task.reqBody.control_filter_to_apply] =
    ...
    taskEntry = imagePreviewContent.insertBefore(
        taskEntry,
        supportBanner.nextSibling
    );
...
function getCurrentUserRequest() {
    ...
    const seed = randomSeedField.checked
    ...
    const newTask = {
        ...
        reqBody: {
            ...
            stream_image_progress:
                numOutputsTotal > 50 ? false : streamImageProgressField.checked,
            ...
            inactive_tags: activeTags
                .filter((tag) => tag.inactive === true)
                .map((x) => x.name),
        },
    };
    if (IMAGE_REGEX.test(initImagePreview.src)) {
        ...
        newTask.reqBody.preserve_init_image_color_profile =
            applyColorCorrectionField.checked;
    ...
    if (useFaceCorrectionField.checked) {
        ...
        if (gfpganModelField.value.includes('codeformer')) {
            newTask.reqBody.codeformer_upscale_faces = document.querySelector(
                '#codeformer_upscale_faces'
            ).checked;
            newTask.reqBody.codeformer_fidelity =
                1 - parseFloat(codeformerFidelityField.value);
        }
    }
    if (useUpscalingField.checked) {
        ...
        if (upscaleModelField.value === 'latent_upscaler') {
            ...
            newTask.reqBody.latent_upscaler_steps =
                latentUpscalerStepsField.value;
        }
    }
    if (hypernetworkModelField.value) {
        newTask.reqBody.use_hypernetwork_model = hypernetworkModelField.value;
        newTask.reqBody.hypernetwork_strength = parseFloat(
            hypernetworkStrengthField.value
        );
    }
    if (testDiffusers.checked) {
        ...
        if (modelNames.length > 0) {
            ...
            modelStrengths =
                modelStrengths.length == 1 ? modelStrengths[0] : modelStrengths;
    ...
    if (
        testDiffusers.checked &&
        document.getElementById('toggle-tensorrt-install').innerHTML ==
            'Uninstall'
    ) {
        // TRT is installed
        newTask.reqBody.convert_to_tensorrt = document.querySelector(
            '#convert_to_tensorrt'
        ).checked;
        let trtBuildConfig = {
            batch_size_range: [
    ...
    if (
        ...
        if (controlImageFilterField.value !== '') {
            newTask.reqBody.control_filter_to_apply =
                controlImageFilterField.value;
        }
    }
...
function setEmbeddings(task) {
    ...
    function extract(entries, basePath = '') {
        entries.forEach((e) => {
            if (Array.isArray(e)) {
                let path =
                    basePath === '' ? basePath + e[0] : basePath + '/' + e[0];
                extract(e[1], path);
            } else {
...
function getPrompts(prompts) {
    ...
    const newTags = activeTags.filter(
        (tag) => tag.inactive === undefined || tag.inactive === false
    );
    if (newTags.length > 0) {
        const promptTags = newTags.map((x) => x.name).join(', ');
        if (promptsToMake.length > 0) {
            promptsToMake = promptsToMake.map(
                (prompt) => `${prompt}, ${promptTags}`
            );
        } else {
...
function updateInitialText() {
    if (document.querySelector('.imageTaskContainer') === null) {
        ...
        const countBeforeBanner =
            localStorage.getItem('countBeforeBanner') || 1;
        if (countBeforeBanner <= 0) {
            supportBanner.classList.remove('displayNone');
        }
    }
...
clearAllPreviewsBtn.addEventListener('click', (e) => {
    shiftOrConfirm(
        e,
        'Clear all the results and tasks in this window?',
        async function () {
            await stopAllTasks();
...
function downloadAllImages() {
    ...
    document.querySelectorAll('.imageTaskContainer').forEach((container) => {
        ...
        container.querySelectorAll('.imgContainer img').forEach((img) => {
            ...
            if (optZIP) {
                let suffix =
                    img.dataset['imagecounter'] + '.' + req['output_format'];
                folder.file(
                    getDownloadFilename(img, suffix),
...
function renameMakeImageButton() {
    let totalImages =
        Math.max(
            parseInt(numOutputsTotalField.value),
            parseInt(numOutputsParallelField.value)
        ) * getPromptsNumber();
...
numOutputsTotalField.addEventListener(
    'keyup',
    debounce(renameMakeImageButton, 300)
);
numOutputsParallelField.addEventListener('change', renameMakeImageButton);
numOutputsParallelField.addEventListener(
    'keyup',
    debounce(renameMakeImageButton, 300)
);
...
function onUpscaleModelChange() {
    ...
    if (
        upscaleModelField.value === 'latent_upscaler' &&
        !upscaleModelField.disabled
    ) {
...
codeformerFidelityField.addEventListener(
    'input',
    updateCodeformerFidelitySlider
);
...
latentUpscalerStepsField.addEventListener(
    'input',
    updateLatentUpscalerStepsSlider
);
...
hypernetworkStrengthSlider.addEventListener(
    'input',
    updateHypernetworkStrength
);
hypernetworkStrengthField.addEventListener(
    'input',
...
hypernetworkModelField.addEventListener(
    'change',
...
outputQualityField.addEventListener(
    'input',
    debounce(updateOutputQualitySlider, 1500)
);
updateOutputQuality();
...
function onAutoScrollUpdate() {
    ...
    autoscrollBtn.querySelector('.state').innerHTML = autoScroll.checked
        ? 'ON'
        : 'OFF';
}
...
function img2imgLoad() {
    ...
    initImageSizeBox.textContent =
        initImagePreview.naturalWidth + ' x ' + initImagePreview.naturalHeight;
...
function selectTab(tab_id) {
    ...
    if (!tabInfo.tab.classList.contains('active')) {
        tabElements.forEach((info) => {
            if (
                info.tab.classList.contains('active') &&
                info.tab.parentNode === tabInfo.tab.parentNode
            ) {
...
function packagesUpdate(event) {
    ...
    let trtInstalled =
        'packages_installed' in event &&
        'tensorrt' in event['packages_installed'];

    if (
        'packages_installing' in event &&
        event['packages_installing'].includes('tensorrt')
    ) {
    ...
    if (
        ...
        document
            .querySelector('#enable_trt_config')
            .classList.remove('displayNone');
        document
            .querySelector('#trt-build-config')
            .classList.remove('displayNone');
    }
}

document
    .getElementById('toggle-cloudflare-tunnel')
    .addEventListener('click', async function () {
        let command = 'stop';
        if (
            document.getElementById('toggle-cloudflare-tunnel').innerHTML ==
            'Start'
        ) {
            command = 'start';
        }
...
document
    .getElementById('toggle-tensorrt-install')
        ...
        shiftOrConfirm(
            ...
            async function () {
                ...
                if (res.status === 'OK') {
                    ...
                    self.innerHTML =
                        command === 'install' ? 'Uninstall' : 'Install';
                } else if (res.status_code === 500) {
                    alert(
                        'TensorselfRT failed to ' + command + ': ' + res.detail
                    );
                    self.innerHTML =
                        command === 'install' ? 'Install' : 'Uninstall';
                }
...
addEmbeddingsThumb.addEventListener('click', (e) =>
    addEmbeddingsThumbInput.click()
);
...
function loadThumbnailImageFromFile() {
    ...
    reader.addEventListener('load', function (event) {
        ...
        onUseAsThumbnailClick(
            {
                use_embeddings_model: getAllModelNames('embeddings').sort(
                    (a, b) =>
...
function updateEmbeddingsList(filter = '') {
    function html(model, iconMap = {}, prefix = '', filter = '') {
            ...
            if (typeof m == 'string') {
                ...
                if (typeof subdir == 'object') {
                    ...
                    div2.replaceChildren(
                        htmlToElement(
                            `<h4 class="collapsible">${prefix}${m[0]}</h4>`
                        ),
                        div1
                    );
        ...
        result.replaceChildren(
            toplevel,
            htmlToElement('<br style="clear: both;">'),
            folders
        );
        return result;
    }
    ...
    function onButtonClick(e) {
        ...
        const insertIntoNegative =
            e.shiftKey ||
            positiveEmbeddingText.classList.contains('displayNone');
    ...
    Bucket.getList(`${profileName}/embeddings/`)
        ...
        .then(async function (icons) {
            ...
            embeddingsList.replaceChildren(
                html(tokenList, iconMap, '', filter)
            );
...
(function () {
    ...
    customWidthField.addEventListener('change', () => {
        ...
        if (w != customWidthField.value) {
            showToast(
                `Rounded width to the closest multiple of ${customWidthField.step}.`
            );
        }
    });
    ...
    customHeightField.addEventListener('change', () => {
        ...
        if (h != customHeightField.value) {
            showToast(
                `Rounded height to the closest multiple of ${customHeightField.step}.`
            );
        }
    });
    ...
    makeImageBtn.addEventListener('click', () => {
        ...
        recentResolutionsValues = recentResolutionsValues.filter(
            (el) => el.w != w || el.h != h
        );
        recentResolutionsValues.unshift({w: w, h: h});
        recentResolutionsValues = recentResolutionsValues.slice(0, 8);

        localStorage.recentResolutionsValues = JSON.stringify(
    ...
    if (_jsonstring == undefined) {
        ...
        recentResolutionsValues = JSON.parse(
            localStorage.recentResolutionsValues
        );
    }
...
document.addEventListener('on_render_task_fail', (e) => {
    ...
    let msg = '';
    if (
        'detail' in stepUpdate &&
        typeof stepUpdate.detail === 'string' &&
        stepUpdate.detail.length > 0
    ) {
        ...
        if (msg.toLowerCase().includes('out of memory')) {
            ...
            msg.includes(
                "RuntimeError: output with shape [320, 320] doesn't match the broadcast shape"
            )
        ) {
            msg += `<br/><br/>
        ...
        msg = `Unexpected Read Error:<br/><pre>StepUpdate: ${JSON.stringify(
            stepUpdate,
            undefined,
            4
        )}</pre>`;
