...
`;
    ...
    let overwriteDuplicatePrompts = document.querySelector(
        '#history-plugin-overwrite_duplicate_prompts'
    );
    ...
    function* getFreeSpace() {
        ...
        do {
            ...
            try {
                trySize =
                    Math.ceil((maxCharSize - minCharSize) / 2) + minCharSize;
    ...
    function setSetup(stateObject) {
        ...
        hypernetworkModelField.value =
            stateObject.hypernetwork === undefined
                ? ''
                : stateObject.hypernetwork;
        hypernetworkStrengthField.value =
            stateObject.hypernetworkStrength === undefined
                ? 0
                : stateObject.hypernetworkStrength;
        document
    ...
    const buildHistoryItem = (timestamp, item) => {
        return `
      <div class="${ID_PREFIX}-history-datetime">${new Date(
            timestamp
        ).toLocaleString()}</div>
      ...
      <span><b>negative:</b> '${item.negative}'</span><br/>
			<span><b>sampler:</b> '${item.sampler}'</span> <span><b>w:</b> '${
            item.width
        }'</span> <span><b>h:</b> '${
            item.height
        }'</span> <span><b>steps:</b> '${
            ...
            item.vae === undefined || item.vae === '' ? 'None' : item.vae
        }'</span><br/>
			<span><b>facefix:</b> '${item.facefix}'</span> <span><b>upscale:</b> '${
            item.useUpscaling
        }'</span> <span><b>Hypernetwork:</b> '${
            item.hypernetwork === undefined || item.hypernetwork === ''
                ? 'None'
                : item.hypernetwork
        }'</span> <span><b>Hypernetwork Strength:</b> '${
            item.hypernetworkStrength === undefined
                ? 0
                : item.hypernetworkStrength
        }'</span> <span><b>seed:</b> ${
            item.random ? 'random' : item.seed
        }</span>
      </div>
    ...
    const loadHistory = () => {
        let historyItemsContainer = document.getElementById(
            `${'history-plugin'}-historyItemsContainer`
        );
        historyItemsContainer.innerHTML = '';
        let historyItems = JSON.parse(
            localStorage.getItem(`${'history-plugin'}-history`)
        );
        ...
        for (let item of historyItems) {
            ...
            currentItem.id = `${'history-plugin'}-history-item-${item.id}`;
            currentItem.classList.add(`${'history-plugin'}-history-item`);
            currentItem.innerHTML = buildHistoryItem(
                item.timestamp,
                item.setup
            );
            ...
            deletebutton.id = `${'history-plugin'}-history-deletebutton-${item.id}`;
            deletebutton.classList.add(`${'history-plugin'}-history-deletebutton`);
    ...
    const saveHistoryItem = () => {
        let currentHistory = JSON.parse(
            localStorage.getItem(`${ID_PREFIX}-history`)
        );
        if (currentHistory.length > 0) {
            ...
            if (overwriteDuplicatePrompts.checked == true) {
                ...
                for (let i = 0; i < currentHistory.length; i++) {
                    ...
                    if (
                        JSON.stringify(currentHistory[i].setup.prompt) ===
                        JSON.stringify(newItem.setup.prompt)
                    ) {
            ...
            if (JSON.stringify(a) !== JSON.stringify(b))
                localStorage.setItem(
                    `${ID_PREFIX}-history`,
                    JSON.stringify([newItem, ...currentHistory])
                );
        } else {
            let newItem = {
                id: 0,
    ...
    if (localStorage.getItem(`${ID_PREFIX}-history`) === null)
    ...
    deleteallbutton.addEventListener('click', (e) => {
        ...
        if (confirmActions.checked !== true) {
            ...
            confirm(
                `Are you sure you want to delete <b><u>all</u></b> items?`,
                `Are you sure you want to delete <b><u>all</u></b> items?`,
                () => {
                    localStorage.removeItem(`${ID_PREFIX}-history`);
                    localStorage.setItem(`${ID_PREFIX}-history`, '[]');
    ...
    function continueStorageUpdate() {
        ...
        const textMsg = `Used: ${formatBytes(usGen.value)} / ${formatBytes(
            usGen.value + fsGen.value
        )}`;
        spacelabel.innerHTML = `<progress id="${ID_PREFIX}-usedspace" class="editor-slider" value="${Math.round(
            (usGen.value / (usGen.value + fsGen.value)) * 100
        )}" max="100" title="${textMsg}">Storage ${textMsg}</progress>`;
    ...
    autosaveentries.checked =
        localStorage.getItem(settings[0].id) == null
            ? settings[0].default
            : localStorage.getItem(settings[0].id) === 'true';
    updateAutosaveListener();
    ...
    function getCurrentTimestamp() {
        ...
        const timestamp = `${shortHours.toString().padStart(2, '0')}.${minutes
            .toString()
            .padStart(2, '0')}${ampm}${month.toString().padStart(2, '0')}/${day
            .toString()
            .padStart(2, '0')}/${year}`;
        return timestamp;
    }
    ...
    function downloadHistory(historyData) {
        ...
        const blob = new Blob([JSON.stringify(historyData)], {
            type: 'text/plain',
        });
    ...
    function loadHistoryFromFile(file) {
        ...
        reader.onload = function (event) {
            try {
                ...
                alert(
                    'Failed to load history. Please ensure the file contains valid JSON data.'
                );
            }
        };
